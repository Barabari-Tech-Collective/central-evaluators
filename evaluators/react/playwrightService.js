 ...
 *
 * @param {string} projectPath - Absolute path to the cloned React project directory
...
export default async function runTests(projectPath) {
  ...
  // spinTestEnvironment recursively uploads a real directory (uploadDirectory
  // in sandboxService.js), not a zip — it previously received the unused
  // submission_zip_path param instead of projectPath, which was always
  // undefined since callers only ever pass one argument, crashing
  // fs.promises.readdir(undefined) on every react job.
